...
import { clientSchema, Client } from "../../data/schema"; // Import Client type
import { useNavigate } from "react-router-dom";


interface DataTableRowActionsProps {
  row: Row<Client>; // Use Client type directly
}

// Component for row actions in the data table
export function DataTableRowActionsclients({
  row,
}: DataTableRowActionsProps) {
  const navigate = useNavigate();

  // Validate the row data using the schema (this is a good practice)
  const client = clientSchema.parse(row.original); // This validates the row data
  ...
  const handleDelete = () => {
    ...
    console.log(`Deleting client with ID: ${client.firstName} ${client.lastName}`);
  };
